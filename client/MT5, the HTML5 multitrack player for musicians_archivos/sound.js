...
function init() {
    ...
    divInfo = document.getElementById("trackInfos");
...
function drawTrack(decodedBuffer, trackNumber) {
    ...
    waveformDrawer.init(decodedBuffer, View.masterCanvas, color);
    var x = 0;
    ...
    View.masterCanvasContext.strokeStyle = "#1c1c1c";
...
function finishedLoading(bufferList) {
    log("Finished loading all tracks, press Play!");
...
function loadSongList() {
    ...
    var defaultSong = whichSession.whichDefault();
    if (defaultSong) loadSong(defaultSong);

    xhr.onload = function (e) {
        var songList = JSON.parse(this.response);
        var whichList = whichSession.whichList();

        var filterList = function (item) {
            if ($.inArray(item, whichList) !== -1) return true;
            return false;
        };

        // Filter song list based on whichSession list.
        if (whichList) {
            songList = songList.filter(filterList);
        }
...
function loadSong(songName) {
    ...
    xhr.onload = function (e) {
        ...
        song.instruments.forEach(function (instrument, trackNumber) {
            ...
            var trackInfoLink = '<a href="#track-info' + (trackNumber+1) + '" class="popup track-name">' + instrument.name + '</a>'

            if (!whichSession.whichInfo(trackNumber)) {
                trackInfoLink = '<p class="track-name-no-link">' + instrument.name + '</p>'
            }
            ...
            span.innerHTML = '<td class="trackBox" style="height : ' + SAMPLE_HEIGHT + 'px">' +
                ...
                '<div class="trackDiv"><div class="trackTitleDiv">' + trackInfoLink + '</div>' +
                "<div class='volumeDiv'><span id='volspan'><input type='range' class = 'volumeSlider custom' id='volume" + trackNumber + "' min='0' max = '100' value='100' oninput='setVolumeOfTrackDependingOnSliderValue(" + trackNumber + ");'/></span></div><td></div>" +
                "<div class='muteSoloButtons'><button class='mute' id='mute" + trackNumber + "' onclick='muteUnmuteTrack(" + trackNumber + ");'><img src='../img/MT5_Mute_Off.jpg' /></span></button> " +
                "<button class='solo' id='solo" + trackNumber + "' onclick='soloNosoloTrack(" + trackNumber + ");'><img src='../img/MT5_Solo_Off.jpg' /></button></div>";

            divTrack.appendChild(span);

            var spanInfo = document.createElement('div');
            spanInfo.setAttribute("id", "track-info" + (trackNumber+1));
            spanInfo.setAttribute("style", "display:none");

            spanInfo.innerHTML = '<p style="margin-bottom: 0;">' + whichSession.whichInfo(trackNumber) + '</p>';
...
function animateTime() {
    ...
    if (currentSong !== undefined) {
        ...
        if (!currentSong.paused) {
            ...
            View.frontCanvasContext.fillText((currentSong.elapsedTimeSinceStart + "").toFormattedTime() + "s", 500, 20);
...
function showWelcomeMessage() {
    ...
    drawArrow(View.frontCanvasContext, 180, 170, 10, 10, 10, 'red');
    drawArrow(View.frontCanvasContext, 450, 80, 450, 10, 10, 'red');WaveformDrawer
...
function soloNosoloTrack(trackNumber) {
    ...
    if (!currentTrack.solo) {
        ...
        s.innerHTML = "<img src='../img/MT5_Solo_On.jpg' />";
    } else {
        // we were in solo mode, let's go to the "no solo" mode
        currentTrack.solo = false;
        // Let's change the icon
        s.innerHTML = "<img src='../img/MT5_Solo_Off.jpg' />";
    }
    ...
    // Let's change the icon
    m.innerHTML = "<img src='../img/MT5_Mute_Off.jpg' />";
...
function muteUnmuteTrack(trackNumber) {
    ...
    if (!currentTrack.muted) {
        ...
        m.innerHTML = "<img src='../img/MT5_Mute_On.jpg' />";
    } else {
